...
function PDPContent({ id }: { id: string }) {
  ...
  return (
    <div className="min-h-screen bg-transparent text-white font-sans">
      ...
      {/* Breadcrumbs */}
      <div className="pt-24 px-4 md:px-8 lg:px-12 pb-4 text-xs tracking-widest text-white/50 flex items-center gap-2 uppercase">
        <Link href="/" className="hover:text-white transition-colors">Home</Link>
        <ChevronRight className="w-3 h-3" />
        <Link href="/" className="hover:text-white transition-colors">Apparel</Link>
        <ChevronRight className="w-3 h-3" />
          ...
          <div className="relative w-full max-w-md mx-auto lg:max-w-lg group">
            ...
            {product.images.length > 1 && (
              <>
                <button
                  ...
                  className="absolute left-2 top-1/2 -translate-y-1/2 z-10 p-2 bg-white/10 hover:bg-white/20 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
                ...
                <button
                  ...
                  className="absolute right-2 top-1/2 -translate-y-1/2 z-10 p-2 bg-white/10 hover:bg-white/20 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
            ...
            >
              {product.images.map((img, idx) => (
                <div key={idx} className="w-full flex-shrink-0 snap-center relative aspect-[4/5] bg-white/5 rounded-md overflow-hidden pointer-events-none border border-white/5">
              ...
              {product.images.map((img, idx) => (
                <button
                  ...
                  className={`relative w-16 h-20 rounded-md overflow-hidden transition-all ${
                    idx === activeSlide ? 'ring-2 ring-cyan ring-offset-2 ring-offset-[#0F172A]' : 'opacity-60 hover:opacity-100'
                  }`}
                >
            ...
            <div className="flex lg:hidden justify-center gap-2 mt-4">
              {product.images.map((_, idx) => (
                <div key={idx} className={`h-1.5 rounded-full transition-all duration-300 ${idx === activeSlide ? 'w-6 bg-cyan' : 'w-1.5 bg-white/20'}`} />
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="w-full lg:w-1/2">
          <div className="lg:sticky lg:top-24 flex flex-col gap-6">
            ...
            <div>
              ...
              <p className="text-sm text-white/50 mt-1">Shipping calculated at checkout.</p>
            </div>

            {/* Mock BNPL Badges */}
            <div className="flex flex-col gap-2 p-4 bg-white/5 rounded-md border border-white/10">
            ...
            {/* Colors */}
            <div>
              <p className="text-sm font-bold uppercase mb-3">Color: <span className="font-normal text-white/70">{product.colors[0]?.name || 'Standard'}</span></p>
              <div className="flex gap-2">
                {product.colors.map((color, idx) => (
                  <div 
                    key={idx} 
                    className="w-10 h-10 rounded-full border border-white/20 p-1 cursor-pointer ring-1 ring-offset-1 ring-offset-[#0F172A] ring-cyan transition-all shadow-[0_0_10px_rgba(56,189,248,0.2)]"
                  >
                    <div 
            ...
            <div>
              <div className="flex justify-between items-center mb-3">
                ...
                <button className="text-xs uppercase underline tracking-widest hover:text-cyan transition-colors">Size Guide</button>
              </div>
              <div className="grid grid-cols-4 md:grid-cols-5 lg:grid-cols-4 gap-2">
                {product.sizes.map((size) => (
                  <button
                    ...
                    className={`py-3 text-sm font-medium uppercase transition-colors border rounded ${
                      selectedSize === size
                        ? "bg-cyan text-navy border-cyan shadow-[0_0_15px_rgba(56,189,248,0.3)]"
              ...
              {sizeError && (
                <p className="text-cyan text-xs mt-2 uppercase font-medium">Please select a size to continue</p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-3 mt-4">
              <button 
                ...
                className="w-full py-4 bg-cyan text-navy rounded-xl font-bold uppercase tracking-widest hover:bg-white transition-colors shadow-[0_0_20px_rgba(56,189,248,0.2)]"
              >
                Add to Cart
              </button>
              <button 
                className="w-full py-4 bg-transparent border border-cyan text-cyan rounded-xl font-bold uppercase tracking-widest hover:bg-cyan/10 transition-colors"
              >
                Buy it Now
              </button>
            </div>
            ...
            <div className="mt-4 border-t border-white/10 divide-y divide-white/10">
              <details className="group">
                ...
                <div className="text-sm text-white/70 pb-4">
              ...
              <details className="group">
                ...
                <div className="text-sm text-white/70 pb-4 leading-relaxed">
                  <p>{product.fabric}</p>
